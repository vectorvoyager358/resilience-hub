...
/** GET an `/api/*` path with the current user's ID token. */
export async function authedGet(path: string, init?: RequestInit): Promise<Response> {
  return authedFetch(apiUrl(path), { ...init, method: 'GET' });
}
...
/**
 * Best-effort GET: never throws for HTTP/network; use when the caller should
 * continue if the backend is unavailable.
 */
export async function authedGetJsonOptional<T extends Record<string, unknown>>(
  path: string
): Promise<{ ok: true; data: T } | { ok: false; status: number; detail: string }> {
  try {
